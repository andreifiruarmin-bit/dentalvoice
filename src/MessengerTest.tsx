...
export default function MessengerTest() {
  ...
  const [userId, setUserId] = React.useState('sim-user-fb-001');
  ...
  const callMessengerApi = async (payload: { text?: string; reset?: boolean }) => {
    const body: Record<string, unknown> = { senderId: userId };
    if (payload.reset) body.reset = true;
    else body.text = payload.text;

    const response = await fetch('/api/messenger/simulate', {
      method: 'POST',
      headers: {
  ...
  const handleReset = async () => {
    ...
    setIsLoading(true);
    try {
      const data = await callMessengerApi({ reset: true });
      appendBotMessage({
        reply: data.reply,
        buttons: data.buttons && data.buttons.length > 0 ? [...data.buttons] : [],
      });
  ...
  const sendUserText = async (raw: string) => {
    ...
    try {
      const data = await callMessengerApi({ text });
      appendBotMessage({
        reply: data.reply,
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4 font-sans">
      <div className="max-w-md w-full">
        <div className="mb-6 bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
          <div className="flex items-start justify-between gap-3 mb-3">
            <label className="block text-xs font-black text-slate-400 uppercase tracking-widest">
              Simulează Facebook User ID
            </label>
            <button
              type="button"
              onClick={handleReset}
              disabled={isLoading}
              className="text-xs font-bold text-[#0084FF] hover:underline disabled:opacity-50 shrink-0"
            >
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center">
              <Facebook className="w-5 h-5 text-[#0084FF]" />
            </div>
            <input
              type="text"
              value={userId}
              ...
              placeholder="Ex: sim-user-fb-001"
            />
          </div>
          <div className="mt-3 flex items-center justify-between gap-2 flex-wrap">
            <p className="text-[10px] text-slate-400 font-medium italic">
              Schimbă ID-ul pentru a simula o sesiune nouă.
            </p>
            <span
        ...
        <div className="bg-[#E4E6EB] h-[700px] rounded-[3rem] shadow-2xl border-[8px] border-slate-900 overflow-hidden flex flex-col relative">
          <div className="bg-[#0084FF] p-4 pt-10 flex items-center justify-between text-white shrink-0">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[9px] opacity-70 truncate font-medium">{userId}</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-white">
            <div className="flex justify-center mb-4">
              <span className="bg-[#E4E6EB] text-[10px] font-bold px-3 py-1 rounded-lg text-slate-600 uppercase tracking-wider">
                Astăzi
              </span>
            </div>
                  ...
                  <div
                    className={`max-w-[80%] p-3 rounded-2xl shadow-sm relative ${
                      msg.sender === 'user'
                        ...
                        : 'bg-[#E4E6EB] text-slate-800 rounded-tl-none'
                    }`}
                  >
                    ...
                    <div className="flex items-center justify-end gap-1 mt-1">
                      ...
                      {msg.sender === 'user' && <CheckCheck className="w-3 h-3 text-blue-100" />}
                    </div>
                  </div>
                  {msg.sender === 'bot' &&
                        ...
                        <div className="flex flex-wrap gap-2">
                          {msg.buttons.map((label) => (
                            <button
                              ...
                              className="px-3 py-1.5 rounded-full text-xs font-bold border-2 border-[#0084FF] text-[#0084FF] bg-white hover:bg-blue-50 disabled:opacity-50 transition-colors shadow-sm"
                            >
                              {label}
                            </button>
                          ))}
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-[#E4E6EB] p-3 rounded-2xl rounded-tl-none shadow-sm flex gap-1">
          ...
          <form onSubmit={handleSend} className="p-3 bg-[#F0F2F5] flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="w-12 h-12 bg-[#0084FF] rounded-full flex items-center justify-center text-white shadow-lg active:scale-95 transition-transform disabled:opacity-50"
            >
        ...
        <div className="mt-8 text-center">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[10px] text-slate-400">DentalVoice Facebook Messenger Bot Simulator v1.0</p>
          </div>
        </div>
      </div>
    </div>
